...
import { setRegistrosEvolucionOdonto } from '../../../appRedux/actions/menu/evolucionTratamientoOdonto';
import { notificaciones, openNotification } from '../../../util/util';
import { traerEvolucionTratamientoOdonto } from './apis';
import { ExclamationCircleOutlined } from '@ant-design/icons';

const EvolucionTratamientoOdonto = ({ datosModal }) => {
	const dataEvolucionTratamientoRedux = useSelector((state) => state.evolucionTratamientoOdonto);
	const registrosEvolucion = useSelector((state) => state.registrosEvolucionOdonto);
	...
	const eliminar = async (record) => {
		...
		const respuesta = await httpClient.post('/consulta/deleteEvolucionTratamientoOdonto', data);
		if (respuesta.data.success) {
			// openNotification('Operacion Exitosa', 'El registro se elimino correctamente', '');
			await traerEvolucionTratamientoOdonto({
	...
	const guardar = async () => {
		...
		setBtnLoading(true);
		const respuesta = await httpClient.post('/consulta/setEvolucionTratamientoOdonto', data);
		...
		if (respuesta.data.success) {
			...
			dispatch(setRegistrosEvolucionOdonto([...arrayTemporal]));
		} else {
			throw 'Por favor vuelva a intentarlo';
				...
				<Col xl={12} lg={24} md={24} sm={24} xs={24}>
					...
					>
						<Input.TextArea disabled={historiaClinica | visualizar}
							onChange={handleTratamiento} rows={4} />
					</Form.Item>
				</Col>
				<Col xl={12} lg={24} md={24} sm={24} xs={24}>
					<h4 style={{ padding: '20px 0 5px 0px', textTransform: 'uppercase' }}>
...
export default EvolucionTratamientoOdonto;
