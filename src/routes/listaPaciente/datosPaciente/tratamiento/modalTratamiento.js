    ...
    useEffect(() => {
        ...
        if (abrirModal) {
            if (dataTratamiento) {
                ...
                const filtroHumanidadStock = dataTratamiento.filter(item => item.NOM_LAB === 'FARMACIA HUMANIDAD SUR' && item.STK_FISICO > 0);
                console.log('filtroHumanidadStock 1', filtroHumanidadStock);

                const filtroHumanidadSinStock = dataTratamiento.filter(item => item.NOM_LAB === 'FARMACIA HUMANIDAD SUR' && item.STK_FISICO < 1);
                console.log('filtroHumanidadSinStock 1', filtroHumanidadSinStock);

                const filtroOtrosStock = dataTratamiento.filter(item => item.NOM_LAB !== 'FARMACIA HUMANIDAD SUR');
    ...
    const columns = [
        {
            ...
            render: (text, record) => {
                return {
                    props: {
                        style: { background: record.NOM_LAB !== "FARMACIA HUMANIDAD SUR" ? colorBlanco : record.STK_FISICO > 0 ? colorVerde : colorRojo }
                    },
    ...
    const filtrarPorOpcion = (e) => {
        if (e.length > 0) {
            if (e[0] === 'farmacia') {
                ...
                //llamar farmacia
                const filterFarmacia = dataTratamiento.filter(data => data.NOM_LAB === 'LIFE CHANGE  180' || data.NOM_LAB.includes("FARMACIA"));
                setFiltro(filterFarmacia);

            } else {
                console.log("SELECCCION OTROS");
                //lamar otros
                const filterOtros = dataTratamiento.filter(data => data.NOM_LAB !== 'LIFE CHANGE  180' && !data.NOM_LAB.includes("FARMACIA"));
        ...
        <>
                    ...
                    <Row style={{ flexDirection: "row" }}>
                        <Col lg={12} md={12} sm={12} xs={12}>
                            <Row style={{ flexDirection: "row", height: "100%" }}>
                                <Col lg={6} md={12} sm={12} xs={12} style={{ flexDirection: "row", display: "inline-flex", gap: '10px', alignItems: "center", justifyContent: "center" }}>
            ...
            >
                {medicamentos === undefined ? (
                    ...
                    <Form ref={formRef} layout="vertical" initialValues={estado} style={{ paddingTop: "20px" }}>
                        <Row
                            style={{
                                flexDirection: "row",
                                marginTop: -15,
