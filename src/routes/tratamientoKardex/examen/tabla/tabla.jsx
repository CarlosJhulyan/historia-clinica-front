...
const TablaExamen = ({ historia, editar, TraerDatos }) => {
  ...
  useEffect(() => {
    if (editar && editar.examenes.length > 0) {
      console.log(editar.examenes, dataExamen);
      const aux = [];
      editar.examenes.forEach(e => {
        // if (e.estado !== '1') {
        aux.push(e);
        // }
      });
      dataExamen.forEach(t => {
        if (!editar.examenes.find(e => e.codigo_producto === t.COD_PROD)) {
          aux.push(t);
        }
      });
      setData(aux);
    } else if (dataExamen.length > 0) {
      setData(dataExamen);
    } else {
      setData([]);
    }
  ...
  const setExamen = async () => {
    ...
    if (validator) {
      var response = {};
      response = await httpClient.post('kardex/setKardexExamen', body);
  ...
  function showConfirm() {
    confirm({
      title: '¿Está seguro de registrar los examenes?',
      ...
      <div style={{ width: '100%' }}>
        <div style={{ marginBottom: '20px' }}>
          ...
          {data.length > 0 ? data.map((item, index) => {
            ...
            <div style={{ display: 'flex', flexDirection: 'row', padding: '50px', fontSize: '14px', justifyContent: 'center', alignItems: 'center' }}>No se encuentran Examenes</div>
          }
        </div>
